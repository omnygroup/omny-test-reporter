...
export const CollectionConfigSchema = z.object({
	...
	rootPath: z.string().optional().describe('Root directory for collection'),
	...
	timeout: z.number().int().positive('Timeout must be positive').default(30000).describe('Timeout in milliseconds'),

	cache: z.boolean().default(false).describe('Enable caching of results'),

	configPath: z.string().optional().describe('Path to config file'),

	ignorePatterns: z.array(z.string()).default([]).describe('Patterns to ignore'),

	eslint: z.boolean().default(true).describe('Enable ESLint checking'),

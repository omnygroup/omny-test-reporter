...
export class FileSystemManager {
	...
	public async initialize(): Promise<void> {
		try {
			...
			logger.error(`Failed to initialize .omnyreporter directories: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
	...
	public async clearPreviousResults(): Promise<void> {
		try {
			...
			logger.error(`Failed to clear previous results: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
	...
	public async writePassedTests(summary: TestResultsSummary): Promise<void> {
		try {
			...
			logger.info(`Wrote ${summary.totalCount} passed tests to .omnyreporter/passed/`);
		} catch (error) {
			logger.error(`Failed to write passed tests: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
	...
	public async writeFailedTests(summary: TestResultsSummary): Promise<void> {
		try {
			...
			const content = JSON.stringify(summary, null, 2);
			await writeFile(summaryPath, content);
			logger.info(`Wrote ${summary.totalCount} failed tests to .omnyreporter/failed/`);
		} catch (error) {
			logger.error(`Failed to write failed tests: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
	...
	public async writeRunSummary(summary: Record<string, unknown>): Promise<void> {
		try {
			...
			logger.error(`Failed to write run summary: ${error instanceof Error ? error.message : 'Unknown error'}`);
		}
	}
